        ...
        <div className="bg-dark-800 border border-dark-600 rounded-xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    <div className="p-4 bg-dark-700 rounded-lg border border-dark-600">
                        ...
                        <div className="space-y-1">
                            <p className="text-white">
                                Volume = <span className="text-primary-400 font-bold">{result.eqVol.toFixed(2)}</span> mL
                            </p>
                            <p className="text-white">
                                pH = <span className="text-primary-400 font-bold">{result.eqPH.toFixed(2)}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 bg-dark-700 rounded-lg border border-dark-600">
                        ...
                        <div className="space-y-1">
                            <p className="text-white">
                                Volume = <span className="text-purple-400 font-bold">{result.eqVol.toFixed(2)}</span> mL
                            </p>
                            <p className="text-white">
                                ΔpH/ΔV = <span className="text-purple-400 font-bold">{result.eqDPHdV.toFixed(2)}</span>
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="p-4 bg-dark-700 rounded-lg border border-dark-600">
                        ...
                        <div className="space-y-1 text-sm">
                            <p className="text-gray-300">
                                จำนวนจุดข้อมูล: <span className="text-white font-medium">{result.volume.length}</span> จุด
                            </p>
                            <p className="text-gray-300">
                                ช่วง Volume: <span className="text-white font-medium">
                                    {result.volume[0].toFixed(2)} - {result.volume[result.volume.length - 1].toFixed(2)}
                                </span> mL
                            </p>
                            <p className="text-gray-300">
                                ช่วง pH: <span className="text-white font-medium">
                                    {Math.min(...result.pH).toFixed(2)} - {Math.max(...result.pH).toFixed(2)}
                                </span>
                            </p>
                        </div>
                    </div>
            ...
            <div className="mt-6 p-4 bg-dark-700 rounded-lg border border-dark-600">
                ...
                <div className="grid sm:grid-cols-2 gap-2 text-sm">
                    <p className="text-gray-300">
                        การทดลอง: <span className="text-white font-medium">{config.expName} {config.expNo}</span>
                    </p>
                    <p className="text-gray-300">
                        ผู้ทดลอง: <span className="text-white font-medium">{config.studentName || profile?.full_name || '-'}</span>
                    </p>
                    <p className="text-gray-300">
                        มหาวิทยาลัย: <span className="text-white font-medium">{profile?.university || '-'}</span>
                    </p>
                    <p className="text-gray-300">
                        วันที่: <span className="text-white font-medium">
